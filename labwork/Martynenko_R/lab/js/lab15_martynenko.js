...
var x = 1;
// т.к. переменная x объявленна, но т.к. присвоение значения x=1 сделанно после вывода, то значение переменной по умолчанию = undefined
...
if ("X" in window) {
    ...
    //undefined т.к. переменная x объявленна, значение переменной по умолчанию = undefined

if ("X" in window) {
        X = 1;
    }
    console.log( X );  //Выведит ошибку, если в объекте window нет свойства X, оно может появится после выполнения команды из 1 примера, тогда и ошибки не будет и выведит 1

if ("X" in window) {
    X = 1;
    }
    var X;
    console.log( X );  // Выведит undefined, т.к. 
